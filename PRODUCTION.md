...
### 5.3 Консистентність RAG-індексу між репліками

Індекс тримається **в памʼяті процесу**, тож `rebuild_rag_index` перебудовує
лише ту репліку, що обробила запит; решта живуть зі старим індексом до власного
лінивого rebuild. Для кількох реплік — патерн **Event-Driven** (див.
`RAG_EMBEDDINGS_PLAN.md` §4):

- write-операція публікує подію `catalog.updated` (Redis pub/sub — інфраструктура
  вже є через `ADD_REDIS_URL`; за великих обсягів — Kafka/RabbitMQ);
- кожна репліка-підписник перебудовує свій індекс → усі консистентні.

На `voyage`-бекенді rebuild дешевий: embeddings кешуються на диску, пере-ембедяться
лише змінені товари. Кеш — per-replica; для спільного — том або Redis.
