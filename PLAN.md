...
- **Білий список полів.** Універсальний `update_product_field` НЕ повинен приймати
  ...
  випадкове псування). Дозволяти лише явний набір: `{"subtitle", "short_description", ...}`.
- **Завжди `ctx.elicit` перед записом** — це єдиний бар'єр між «LLM щось вирішила»
  і «дані змінилися».
- **Читати перед записом** — показувати у підтвердженні `було → стане`, щоб людина
  бачила, що саме змінюється.
- **Масові операції (`bulk_*`)** — показувати у запиті підтвердження, *скільки рядків*
